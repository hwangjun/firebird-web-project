...
import Partner from "../../models/product/Partner";
...
router.get("/", (req, res) => {
  Partner.findAll()
    .then(partner => {
      if (!partner.length)
        return res.status(404).send({ err: "Partner not found" });
      res.send(partner);
    })
...
router.get("/:partnerCode", (req, res) => {
  Partner.findByPartnerCode(req.params.partnerCode)
    .then(partner => {
